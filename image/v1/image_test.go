...
func TestListImages(t *testing.T) {
	...
	testImageServiceAction(t, "images", hpHelionSampleImagesData, anon)
}
...
func TestListImageDetails(t *testing.T) {
	anon := func(imageService *image.ImageService) {
		...
		expectedImageDetail := image.ImageDetailResponse{
			...
			Deleted:         false,
			ContainerFormat: "bare",
			CreatedAt:       "2014-09-29T14:44:31",
			DiskFormat:      "qcow2",
			UpdatedAt:       "2014-09-29T15:33:37",
			MinDisk:         8,
			Protected:       false,
	...
	testImageServiceAction(t, "images/detail", hpHelionSampleImageDetailsData, anon)
}
...
func testImageQueryParameter(t *testing.T, uriEndsWith string, queryParamAction func(*image.ImageQueryParameters)) {
	...
	testImageServiceAction(t, uriEndsWith, hpHelionSampleImagesData, anon)
}
...
var hpHelionSampleImageDetailsData = `{
   "images":[
      {
         "status":"active",
         "name":"Ubuntu Server 12.04.5 LTS (amd64 20140927) - Partner Image",
